...
import {setCriteriaDefinitions, setCurrentTemplate, setResultSetData} from "../../../slices/dashboardSlice";
...
const ContentSearchComponent=()=>{
    const {hasValid, noContext,criteriaDefinitionsData,resultSetDefinitionsData,loading,resultSetData,currentTemplate,defaultTemplate, totalSearches}=useSelector((state) => state.dashboard);
    ...
    const headerWSIcon = <Icons icon={ICONS.DOC_CONTENT_SEARCH} />;
    ...
    (currentTemplate.reassignEnabled) && resultButtonActions.push(<Button icon={reassignIcon} disabled={!selectedRows?.length} text={t('btn.searchbutton.reassign')} title={t('btn.searchbutton.reassign')} className={styles.buttonIcon} />);
    ...
    const handleSearchClick=(flag)=>{
        if(hasValid || flag){
            getSearchData('content','0-24', currentTemplate )
        }else{
            ...
            if (isSortApplied) {
                if (totalSearches > 100) {
                    ...
                    try {
                        ...
                        await getSearchData('content',`${range.startPage === 1 ? 0 : range.startPage}-${range.endPage}`, currentTemplate, selectedColumn, selectedSort);
                    } catch (error) {
                        setIsError(true);
                ...
                await getSearchData('content',`${range.startPage ===  1 ? 0 : range.startPage}-${range.endPage}`, currentTemplate);
            }
        }
    }
        ...
        <ReflexElement flex={0.2} style={{maxHeight:'30px'}} >
            <div className={styles.header}>
                <div className={styles.headerContainerStyle}>
                    ...
                    <h4 className={styles.headerContent}>{currentTemplate.name?t('sysmgr.broker.attachment.contentsearchs.label')+' : '+currentTemplate.name:''}</h4>
                </div>
